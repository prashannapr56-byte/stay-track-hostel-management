...
router.post('/rooms/:roomId/students/quick-add', (req, res) => {
  const roomId = Number(req.params.roomId)
  const { registerNumber, name, department, studentPhone, parentPhone } = req.body || {}
...
router.patch('/complaints/:id', (req, res) => {
  ...
  db.prepare('UPDATE complaints SET status = ?, updated_at = ? WHERE id = ?').run(status, now, id)
  const row = db
    .prepare(
      `SELECT c.*, s.name as student_name, s.register_number
